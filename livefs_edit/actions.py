...
def install_debs(ctxt, debs: List[str] = ()):
    ...
    for i, deb in enumerate(debs):
        ...
        ctxt.run(['chroot', rootfs, 'apt', 'install', 'gdebi-core', '-y'])
        ctxt.run(['chroot', rootfs, 'gdebi', '-n', deb_name])
...
@register_action()
def install_packages_complete(ctxt, packages: List[str], repositories: List[str], gpgkeys: List[str], pip_list=None, flavor=None):
    key_folder_path = "/ws-auto/keys/"
    base = ctxt.edit_squashfs(get_squash_names(ctxt)[0])
    keys = []
    print("Adding gpg keys...")
    for key in gpgkeys:
        key_name = key_folder_path + key.split("/")[-1]
        cp2(key, base+key_name)
        keys.append(key_name)
    if gpgkeys:
        env = os.environ.copy()
        ctxt.run(['chroot', base, 'apt-key', 'add'] + keys, env=env)
        ctxt.run(['chroot', base, 'apt-get', 'update'], env=env)
    print("Adding repositories to the sources.list file...")
    for repo in repositories:
        add_apt_repository2(ctxt, repo, base)
    ctxt.run(['chroot', base, 'apt-get', 'update'])
    env = os.environ.copy()
    env['DEBIAN_FRONTEND'] = 'noninteractive'
    env['LANG'] = 'C.UTF-8'
    ctxt.run(['chroot', base, 'apt-get', 'install', '-y'] + packages, env=env)
    if len(pip_list) != 0:
        ctxt.run(['chroot', base, 'pip3', 'install'] + pip_list, env=env)
    if flavor is not None:
        replace_kernel(ctxt, base, flavor)


@register_action()
def install_pip_packages(ctxt, packages: List[str]):
    env = os.environ.copy()
    env['DEBIAN_FRONTEND'] = 'noninteractive'
    env['LANG'] = 'C.UTF-8'
    base = ctxt.edit_squashfs(get_squash_names(ctxt)[0])
    ctxt.run(['chroot', base, 'pip3', 'install'] + packages, env=env)


@register_action()
def add_apt_repository(ctxt, repositories: List[str]):
    base = ctxt.edit_squashfs(get_squash_names(ctxt)[0])
    for repo in repositories:
        ctxt.run(['chroot', base, 'add-apt-repository', '-y', repo])


def add_apt_repository2(ctxt, repo, base):
    ctxt.run(['chroot', base, 'add-apt-repository', '-y', repo])
